...
static Channel *to_tfs, *from_tfs;
int pid = -1;
int serious_errs;
int minor_errs;
char home[20];
...
void test_connect()
{
	...
	pid = tfs_connect (to_tfs, from_tfs, 7, 12, 0);
	if (pid == -1)
	{
		fatal_error("logon failed");
	}

	rv = tfs_chdir(pid, home);
	if (rv == -1)
	{
		rv = tfs_mkdir(pid, home, 0777);
...
void test_fstat()
{
	...
	if (memcmp(&st, &fst, sizeof(struct tfs_stat_t)) != 0)
	{
...
{
   int user = *( (int*)get_param(5) );

   from_tfs = (Channel*) get_param(3);
   to_tfs = (Channel*) get_param(4);

   sprintf(home, "/fulltest%d", user);
   printf("fulltest %d running\n", user);
